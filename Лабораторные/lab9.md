...
1. Замыкание для кэширования: Реализуй проверку на простоту с помощью лямбда-функции с захватом. Лямбда должна захватывать по ссылке внешний unordered_map (кэш), где ключ - число, значение - булево (простое/нет). Перед вычислением лямбда должна проверять, есть ли число в кэше.
2. Оптимизированное удаление: Используй эту лямбду в алгоритме удаления из Базового Варианта (шаг 3). Сравни производительность (по количеству операций) до и после использования кэша для большого набора данных (можно увеличить N до 1000).
3. Генератор с замыканием: Создай лямбда-функцию-генератор, которая захватывает по ссылке внешний генератор случайных чисел и предыдущее сгенерированное значение. При каждом вызове она должна возвращать следующее число Фибоначчи. Используй ее с generate_n, чтобы заполнить vector первыми 15 числами Фибоначчи.
